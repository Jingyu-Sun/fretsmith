...
const icon = {
  ...
  waveform: `
    <svg class="toolbar-svg" viewBox="0 0 24 24" aria-hidden="true">
      <path d="M2 12h2l2-4 2 8 2-6 2 4 2-2 2 6 2-8 2 4h2" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" />
    </svg>`,
}
  ...
  <div class="app-shell">
    ...
    <footer class="transport-bar transport-bar-jitashe">
      ...
      <div class="transport-settings">
        ...
        <button id="mp3-toggle" class="toolbar-icon-button toolbar-action-icon ${state.waveformVisible ? 'is-active' : ''}" title="MP3 audio sync" ${state.isLoaded ? '' : 'disabled'}>
          <span class="toolbar-symbol" aria-hidden="true">${icon.waveform}</span>
